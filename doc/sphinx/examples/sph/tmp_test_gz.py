...
for N_target in [1e3, 1e4, 1e5, 1e6]:
    for split_fact in [0.5, 0.9, 1, 1.1, 8]:
        ...
        results[N_target]["rate"].append(model.solver_logs_last_rate())
        results[N_target]["cnt"].append(model.solver_logs_last_obj_count())
        results[N_target]["patch_count"].append(model.get_patch_count())
