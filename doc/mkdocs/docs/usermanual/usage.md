...
Another possibility (the one that is the most used also) is to start a runscript, which is just a python script starting that will be executed by shamrock, for example this is a very basic script that gets the SPH model with M6 kernel and just starts the patch scheduler on it.
