...
NEW_TEST(Unittest, "shamsolvergraph/LifetimeTracker", 1) {
...

    {
        ...
        // Step 2: creating a node fires exactly one create event for that node. A self
        // state_update is not required at construction: INode cannot emit one from its own
        // ctor (typeid() would report INode), and this node has not rebound edges yet.
        ...
        // the start and op_id 1 at the end. A state_update was already recorded at bind, so
        // evaluate() must not emit another one (ensure_state_update is a no-op then).
        ptr->evaluate();
...
NEW_TEST(Unittest, "shamsolvergraph/LifetimeTracker_OperationSequence", 1) {
    ...
    // Step 4: wrapping the child into an OperationSequence fires one create event. A sequence
    // owns no ro/rw edges of its own, so it does not go through set_edges, and its constructor
    // must not fire a state_update (INode's ctor would misreport typeid as INode).
    auto seq
    ...
    // Step 5: the rule under test - a state_update must be recorded before evaluate_begin, with
    // the genuine OperationSequence dynamic type (not INode). INode::evaluate() lazily emits
    // that update on first call when none was recorded yet, after the derived object is
    // complete so typeid() is correct. A ctor-time INode notify would either show up in step 4
    // or report the wrong dynamic_type here.
    seq->evaluate();
    expected.push_back(
        {{"event", "state_update"},
         {"type", "INode"},
