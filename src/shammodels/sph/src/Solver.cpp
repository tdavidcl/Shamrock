                        ...
                        sham::kernel_call_hndl(
                            ...
                            [corrected_len, group_size, G, gpart_mass, gravitational_softening](
                                ...
                                return [=](sycl::handler &cgh) {
                                    ...
                                    // here the cheat is to disable particles by setting their mass
                                    // to 0 since we will load particles in groups of groups_size
                                    // which exceeds the number of particles for the last pass
                                    auto position_scratch = sycl::local_accessor<vec4>{
                                        sycl::range<1>{group_size}, cgh};

                                    cgh.parallel_for(range, [=](sycl::nd_item<1> tid) {
                                        u64 global_id = tid.get_global_linear_id();
                                        u64 local_id  = tid.get_local_linear_id();
                                        ...
                                        for (size_t offset = 0; offset < Npart;
                                             offset += group_size) {
