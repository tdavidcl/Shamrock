...
struct AlphaGetterVarying {
    sham::DeviceBuffer<Tscal> &buf_alpha_AV;

    struct accessed {
        const Tscal *alpha_AV;

        Tscal operator()(u32 i) const { return alpha_AV[i]; }
    };

    accessed get_read_access(sham::EventList &depends_list) {
        auto alpha_AV = buf_alpha_AV.get_read_access(depends_list);
        return accessed{alpha_AV};
    }

    void complete_event_state(sycl::event e) { buf_alpha_AV.complete_event_state(e); }
};

template<class Tscal>
struct AlphaGetterConstant {
    Tscal alpha_AV;

    struct accessed {
        Tscal alpha_AV;

        Tscal operator()(u32 i) const { return alpha_AV; }
    };

    accessed get_read_access(sham::EventList &depends_list) { return accessed{alpha_AV}; }
...
void shammodels::sph::modules::UpdateDerivs<Tvec, SPHKernel>::update_derivs_hydro_artif_visc(
    ...
    scheduler().for_each_patchdata_nonempty([&](Patch cur_p, PatchData &pdat) {
        ...
        auto alpha_getter = alpha_getter_gen(storage.alpha_av_ghost, cur_p.id_patch);
        ...
        auto alpha_AV   = alpha_getter.get_read_access(depends_list);
        ...
        auto e = q.submit(depends_list, [&](sycl::handler &cgh) {
            ...
            const Tscal alpha_u = _alpha_u;
            const Tscal beta_AV = _beta_AV;
        ...
        alpha_getter.complete_event_state(e);
...
void shammodels::sph::modules::UpdateDerivs<Tvec, SPHKernel>::update_derivs() {
    ...
    if (None *v = std::get_if<None>(&cfg_av.config)) {
        ...
        update_derivs_hydro_artif_visc(
            [&](auto &storage_comp_alpha, u64 id_patch) {
                return AlphaGetterConstant<Tscal>{v->alpha_AV};
            },
            v->beta_AV,
            v->alpha_u);
        ...
        update_derivs_hydro_artif_visc(
            [](auto &storage_comp_alpha, u64 id_patch) {
                return AlphaGetterVarying<Tscal>{storage_comp_alpha.get().get(id_patch).get_buf()};
            },
            v->beta_AV,
            v->alpha_u);
