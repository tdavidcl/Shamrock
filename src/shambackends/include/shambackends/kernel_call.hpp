...
namespace sham {
    ...
    namespace details {
        ...
        /// internal implementation of kernel_call_hndl without a thread count, for kernel
        /// generators that build their own range/nd_range internally and have no use for `n`
        template<class RefIn, class RefOut, class Functor>
        void kernel_call_lambda(
            sham::DeviceQueue &q,
            RefIn in,
            RefOut in_out,
            Functor &&kernel_gen,
            ...
            std::apply(
                [&](auto &...__acc_in) {
                    std::apply(
                        [&](auto &...__acc_in_out) {
                            // submit the kernel generated by the functor
                            e = q.submit(depends_list, kernel_gen(__acc_in..., __acc_in_out...));
                        },
                        acc_in_out);
                },
                acc_in);
    ...
    /// version of kernel_call_hndl without a thread count, for kernel generators that build
    /// their own range/nd_range internally (e.g. from a size that isn't a simple element count)
    template<class RefIn, class RefOut, class Functor>
    void kernel_call_hndl(
        sham::DeviceQueue &q,
        RefIn in,
        ...
        details::kernel_call_lambda<RefIn, RefOut>(
            q, in, in_out, std::forward<Functor>(kernel_gen));
    }
