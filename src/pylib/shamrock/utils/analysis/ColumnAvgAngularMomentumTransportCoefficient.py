...
class ColumnAvgAngularMomentumTransportCoefficient:
    def __init__(
        self,
        model,
        ext_r,
    ...
    def compute_angular_momentum_transport_coefficient(self):
        ...
        def custom_getter(index, dic_out):
            ...
            e_r = np.array([x, y, 0])
            e_r /= np.linalg.norm(e_r) + 1e-9  # Avoid division by zero
    ...
    def analysis_save(self, iplot):
        arr_v_z = self.compute_angular_momentum_transport_coefficient()
    ...
    def plot(
        ...
        **kwargs,
        ...
        if shamrock.sys.world_rank() == 0:
            arr_angular_momentum_transport_coefficient, metadata = self.load_analysis(iplot)
            ...
            res = plt.imshow(
                arr_angular_momentum_transport_coefficient,
                cmap=my_cmap,
                origin="lower",
            ...
            cmap_label = "$\\langle \\alpha \\rangle$ [unitless]"
    ...
    def render_gif(self, save_animation=False):
        if shamrock.sys.world_rank() == 0:
            ...
            if save_animation:
                ...
                ani.save(
                    self.helper.analysis_prefix
                    + "angular_momentum_transport_coefficient_column_avg.gif",
                    writer=writer,
                )
            return ani
        return None
