...
class SliceAlphaAVPlot:
    ...
    def compute_alpha_AV_xy(self):
        arr_alpha_AV_xy = self.helper.slice_render(
            "alpha_AV", "f64", self.do_normalization, self.min_normalization
        )
    ...
    def render_gif(self, save_animation=False):
        if shamrock.sys.world_rank() == 0:
            ...
            if save_animation:
                ...
                ani.save(self.helper.analysis_prefix + "alpha_AV_slice.gif", writer=writer)
            return ani
        return None
