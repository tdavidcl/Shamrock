...
class ColumnAvgAlphaAVPlot:
    ...
    def compute_alpha_AV_xy(self):
        arr_alpha_AV_xy = self.helper.column_average_render(
            "alpha_AV", "f64", self.min_normalization
        )

        return arr_alpha_AV_xy

    def analysis_save(self, iplot):
        arr_alpha_AV_xy = self.compute_alpha_AV_xy()
        self.helper.analysis_save(iplot, arr_alpha_AV_xy)
    ...
    def plot(
        ...
        **kwargs,
        ...
        if shamrock.sys.world_rank() == 0:
            arr_alpha_AV_xy, metadata = self.load_analysis(iplot)
            ...
            res = plt.imshow(
                arr_alpha_AV_xy, cmap=my_cmap, origin="lower", extent=metadata["extent"], **kwargs
            )
            ...
            cmap_label = "$\\langle \\alpha_{AV} \\rangle$ [unitless]"
    ...
    def render_gif(self, save_animation=False):
        if shamrock.sys.world_rank() == 0:
            ...
            if save_animation:
                ...
                ani.save(self.helper.analysis_prefix + "alpha_AV_column_avg.gif", writer=writer)
            return ani
        return None
