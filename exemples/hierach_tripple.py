...
# hierarichle split
split_list = [
    {"index" : 0, "mass_ratio" : 0.5, "a": 1, "e":0.},
    {"index" : 0, "mass_ratio" : 0.5, "a": 0.33333333, "e":0.}
]
...
def split_as_binary(sink,m1, m2, a, e):
    ...
    s1 = {
        "mass": m1,
        "racc": sink["racc"],
        "pos" : (sink["pos"][0] + r1[0],sink["pos"][1] + r1[1],sink["pos"][2]),
        "vel" : (sink["vel"][0] + v1[0],sink["vel"][1] + v1[1],sink["vel"][2])}

    s2 = {
        "mass": m2,
        "racc": sink["racc"],
        "pos" : (sink["pos"][0] + r2[0],sink["pos"][1] + r2[1],sink["pos"][2]),
        "vel" : (sink["vel"][0] + v2[0],sink["vel"][1] + v2[1],sink["vel"][2])}
    ...
    for split in split_list:
        ...
        for i in range(len(sink_list)):
            if i == index_split:
                ...
                s1,s2 = split_as_binary(sink_list[i], smass*mass_ratio, smass*(1-mass_ratio), asplit, esplit)
...
def plot_rho_integ(ext,sinks,arr_rho, iplot):
    ...
    res = plt.imshow(arr_rho, cmap=my_cmap,origin='lower', extent=[-ext, ext, -ext, ext], norm="log", vmin=1e-9, vmax=1e-5)
